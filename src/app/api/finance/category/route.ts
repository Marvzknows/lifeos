...
import { createFinanceCategory, getFinanceCategoriesList } from "@/lib/server/services/finance-category-service";
...
export async function GET(request: NextRequest) {
    try {
        const userId = getCurrentUserId(request);
        const categories = await getFinanceCategoriesList(userId);
